...
- The workflow fetches deploy credentials from Infisical path `/forgejo/parle-landing-ci` using the dedicated `INFISICAL_PARLE_LANDING_CI_ACCESS_TOKEN` bootstrap secret and the runner-local tailnet URL `https://parle-vault.quoll-altair.ts.net`.
...
3. Store the Infisical machine token as the Forgejo Actions secret `INFISICAL_PARLE_LANDING_CI_ACCESS_TOKEN` on `parlehq/parle-landing`.
